...
export class Analytics {
    ...
    async retrieve(namespace: string, date: string) {
        ...
        return {
            date,
            events: Object.entries(res ?? {}).map(([key, value]) => ({[key]: value})),
        }
    }

    public static async retrieveDays() {
        // Fetch and return analytics data for past days
        // Return type could be something like { date: string; views: number }[]
